...
def tokenize(df_train, df_valid, df_test):  # change no context or context the column: Text_x, combined_text
    tokenized_train = df_train['Text_x'].apply(lambda x: tokenizer.encode(x, add_special_tokens=True, max_length=512, truncation=True))
    tokenized_valid = df_valid['Text_x'].apply(lambda x: tokenizer.encode(x, add_special_tokens=True, max_length=512, truncation=True))
    tokenized_test = df_test['Text_x'].apply(lambda x: tokenizer.encode(x, add_special_tokens=True, max_length=512, truncation=True))
...
df_test['Prediction_x'] = make_predictions(test_dataloader)
...
print("Test Accuracy: ", accuracy_score(df_test['Label_x'], df_test['Prediction_x']))
